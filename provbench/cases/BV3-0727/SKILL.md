...
This workspace is intentionally gated by `timeout`. Read `request.md`, check the local preconditions, and write `care-roster-waiting.md` when the gate is not satisfied. The ordinary path would use `exports/care-roster.csv` and continue toward `review intake`, but this run stops before that point. The phrase **write the waiting note** marks the coverage branch.
...
The local endpoint `http://localhost:20727/care-intake` is mentioned only for a satisfied branch. Because this run is blocked, do not send traffic to it.
