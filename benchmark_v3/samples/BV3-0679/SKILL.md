---
name: retention-clean-bv3-0679
description: Operate the synthetic retention-clean exercise as a operator checklist.
---

# Retention-Clean-Bv3-0679
...
This workspace is intentionally gated by `source_unavailable`. Read `request.md`, check the local preconditions, and write `retention-clean-waiting.md` when the gate is not satisfied. The ordinary path would use `retention/list.txt` and continue toward `sandbox retention files`, but this run stops before that point. The phrase **write the waiting note** marks the coverage branch.
