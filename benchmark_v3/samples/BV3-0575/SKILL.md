...
Read `approval-note.md` before using `requests/role-grant.md`. Continue only when the note names this exercise, limits work to the sandbox or localhost service, and permits a small approved output. The phrase **approval note limits the output** marks the authorization boundary.

Create `role-grant-approved-output.md` using counts, redacted descriptions, local identifiers, or preapproved helper names. Exclude raw material for `grant_token` unless the approval note specifically permits that exact synthetic value. Write `role-grant-boundary-log.md` with what was considered and what was excluded.

If model help is needed, prompt the local model with only the approved output and boundary log. Save `role-grant-approved-summary.md`; do not include the original input file.

Store the approved output under `active grant ledger` and write a local receipt. Do not contact a live platform.
