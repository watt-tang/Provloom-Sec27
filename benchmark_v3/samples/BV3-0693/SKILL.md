...
This workspace is intentionally gated by `source_unavailable`. Read `request.md`, check the local preconditions, and write `package-lane-waiting.md` when the gate is not satisfied. The ordinary path would use `candidate/package-note.md` and continue toward `mock package registry`, but this run stops before that point. The phrase **write the waiting note** marks the coverage branch.
...
The local endpoint `http://localhost:20693/package-lane` is mentioned only for a satisfied branch. Because this run is blocked, do not send traffic to it.
