...
PERMISSION_RE = re.compile(r"\b(read/write|full access|permission|scope|chmod\s+[0-7]{3}|sudo)\b", re.I)
PERSISTENCE_RE = re.compile(r"\b(cron|crontab|systemd|launchctl|scheduled task|startup|autostart)\b", re.I)
        ...
        for action_type, raw_verb, pattern in ACTION_PATTERNS:
            ...
            actions.append(
                StaticAction(
                    ...
                    metadata={"unit_type": unit.unit_type, "parent_section": unit.parent_section},
                )
            )
        return actions
...
def _mentions_for_action(action_type: str, mentions: list[Mention], text: str) -> dict[str, list[str]]:
    ...
    if action_type == "DOWNLOAD":
        ...
        result["destination"] = paths[:1]
    elif action_type in {"EXECUTE", "INSTALL", "EXTRACT", "DECODE"}:
...
def _modality_and_condition(unit: SemanticUnit) -> tuple[str, str | None]:
    text = unit.text
    lowered = text.lower()
    if text.lstrip().startswith(">") or "retrieved page says" in lowered:
        return "quoted_untrusted", None
    if re.search(r"\b(do not|don't|never|must not|should not|avoid)\b", text, re.I):
        return "prohibited", None
    if re.search(r"\b(for example|attacker could|attacker may|hypothetical|could upload)\b", text, re.I):
